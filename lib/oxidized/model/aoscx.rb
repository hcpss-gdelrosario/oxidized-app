class Aoscx < Oxidized::Model
  ...
  cmd 'show environment' do |cfg|
    ...
    # match Total Power Consumption (3 digits) and keep table formatting:
    cfg.gsub! /^([t|T]otal\s[p|P]ower\s[c|C]onsumption\s+)(\d{3}\.\d\d)$/, '\\1<power>'
    # match Total Power Consumption (4 digits) and keep table formatting:
    cfg.gsub! /^([t|T]otal\s[p|P]ower\s[c|C]onsumption\s+)(\d{4}\.\d\d)$/, '\\1 <power>'
    # match Total Power Consumption (5 digits) and keep table formatting:
    cfg.gsub! /^([t|T]otal\s[p|P]ower\s[c|C]onsumption\s+)(\d{5}\.\d\d)$/, '\\1  <power>'
    comment cfg
  end
